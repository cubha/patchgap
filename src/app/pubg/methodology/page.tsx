...
export default function PubgMethodologyPage() {
  ...
  // 검증 통과 원인 수 — 화면이 말하는 수는 **데이터에서** 나온다(리터럴로 적으면 재생성에 뒤처진다).
  const verifiedCauseCount = deltas.rows.reduce(
  ...
  const slots: MethodologySlots = {
    sample: (
      <div className="flex flex-col gap-5 p-5">
        ...
        {/* 기저 이동 — 점유율은 상대값이라 "다 같이 움직인 만큼"을 빼지 않으면 착시가 생긴다.
            표본과 같은 질문(무엇을 셌나)에 답하므로 이 슬롯 안에 둔다. */}
        <div className="border-t border-border-soft pt-4">
          <p className="font-body text-xs font-bold text-muted">함께 움직인 값</p>
          <div className="p-5">
    ...
    display: (
      <div className="p-5">
        ...
        <p className="mt-4 text-xs leading-relaxed text-muted">
          이 패치에서 화면에 올리지 않은 관측은 {hidden}종 / 전체 {deltas.meta.n}종입니다.
        </p>
      </div>
    ),
    // 규칙을 먼저 적고 **이 패치의 실측**을 뒤에 붙인다 — "후보 조항이 없다"처럼 데이터에
    // 의존하는 단언을 문장에 고정하면, 다음 패치에서 화면이 거짓을 말한다(K2-7과 같은 실패).
    cause: (
      <div className="flex flex-col gap-3 p-5 text-sm leading-relaxed text-fg-2">
        <p>
          원인 추정은 <strong className="text-fg">같은 패치의 다른 조항이 이 변화를 설명하는가</strong>를
          묻는 것입니다. 그 무기 자신의 조항(직접 변경)은 앞 단계인 결정론 짝짓기가 이미 처리하므로
          후보에서 빠지고, 남는 것은 간접 원인뿐입니다. 인용한 항목 id가 실재하지 않으면 기계가
          기각하고, 기각된 문장은 링크 없이 회색으로만 남습니다.
        </p>
        <p>
          이 패치({deltas.meta.to})의 실측: 무기 관련 조항 <strong className="text-fg">{notes.length}건</strong>,
          검증을 통과한 원인 <strong className="text-fg">{verifiedCauseCount}건</strong>입니다.
          ...
          {verifiedCauseCount === 0 ? (
            <>
              {" "}
              후보로 쓸 조항이 적어 짚을 간접 원인이 나오지 않았습니다 — 후보가 없는데 문장을 만들면
              그것은 추정이 아니라 창작이므로, 무기 상세에는 관측값과 판정 근거만 둡니다.
            </>
          ) : null}
        </p>
      </div>
    ...
    notify: (
      <div className="flex flex-col gap-3 p-5 text-sm leading-relaxed text-fg-2">
        ...
        <p>
          채널은 <strong className="text-fg">게임마다 따로</strong>입니다(배틀그라운드 전용 웹훅) —
  ...
  return (
    <MethodologyLayout
      ...
      lead={
        <>
          {deltas.meta.to} 패치노트의 무기 변경을 매치 텔레메트리의{" "}
          <strong className="text-fg">획득 점유율</strong> 한 축으로 대조합니다. 효과크기 바닥은 이
          데이터에서 유도하고, 판정이 서지 않는 관측은 화면에 올리지 않습니다.
        </>
      }
