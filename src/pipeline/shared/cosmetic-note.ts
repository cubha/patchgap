...
// 것이라 **치장으로 접으면 안 된다**. 그래서 비계량 조건은 **필요조건**으로만 쓰고
// (진짜 밸런스 변경을 실수로 숨기지 않기 위한 안전장치), 치장 증거를 따로 요구한다.
// ⚠️ 2026-09-18(S5) 단서: 이 결정은 **"치장으로 접지 말라"**까지이고, 그 줄에 붙는 **문구**가
// "관측 보류"여야 한다는 뜻은 아니었다. 「의회 - 투표 1 결과」에는 끝내 관측이 오지 않으므로
// "보류"(곧 온다는 약속)는 지켜지지 않는다 — 배지는 그대로 두되 어휘를 사실 서술
// `unpaired`("짝지은 관측 없음")로 바꿨다. 접지 않는다는 이 파일의 판정은 그대로다.
