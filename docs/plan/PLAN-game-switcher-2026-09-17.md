...
### 4-3. 램프 값은 문서 값을 출발점으로 쓰되 재도출한다

HANDOFF §3이 스스로 경고한다 — "PUBG 중립 램프는 **V4 확정 이전에 잡은 값**이다. 실제로
붙일 때 V4 기준으로 재도출할 것". 문서 값(`--bg:#0b0906 --surface:#1a150d …`)을 출발점으로
쓰고 HANDOFF §7 대비 게이트(본문 4.5:1 · 비텍스트 3.0:1)를 새 surface 기준으로 실측한다.
미달 토큰만 조정하고 무엇을 왜 바꿨는지 남긴다.
...
처방은 §5 ST-6에서 실측으로 확정한다(추정으로 또 고치지 않는다 — 3회 재발한 이유).
