...
4. 모바일 레일에 **스크롤바가 안 보이는 환경**(overlay scrollbar)에서는 다음 카드의 28px 노출이
   ...
   보정하지는 않는다.
5. **디스코드 브리핑의 미공지 목록은 `indirect-effect`를 뺀다** — `webhook.ts:149`가
   `r.status === "unannounced"`로 거른다. 헤드라인 수치는 `countGapEntities`(= `isGapStatus`,
   두 상태 포함)를 쓰므로 목록과 머리 수의 근거가 다르다. **LoL에서 이미 그렇게 동작하고
   있었고**, 고치면 9/24 무인 실행의 브리핑 내용이 바뀐다(예선 심사 기간). 이번 라운드에서는
   건드리지 않고 사실만 적어 둔다 — 웹은 접고 디스코드는 안 접는다.
6. PUBG 텔레메트리 다운로드가 매치당 16~30MB라 **CI 러너 대역폭·시간**을 실제로 얼마나 쓰는지는
