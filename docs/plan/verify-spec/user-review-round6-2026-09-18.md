...
## FIX-4 (2026-09-19 — 델타 재채점 96/100 잔여 2건)
- 구현 결정: `display-normalize.ts` — `llm.summaryCites`에 제외 노트가 있으면 `summaryVerified:false`(상세 원인 카드 요약이 `--muted`). `/pubg/` 히어로 캡션 "KR 스쿼드" → "Steam · 전 지역·전 티어"(헤더·방법론과 같은 표본 표기; PUBG API는 지역 샤드가 없다).
- 미확인 사항: K4-3(LLM 원인 문장) 보류 그대로.
