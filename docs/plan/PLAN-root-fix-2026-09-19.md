...
- **이번 라운드 신규 거부**(근거 `BRAINTRUST-root-fix-2026-09-19.md` §0-1·§5):
  ...
  - ~~**`toCandidateView`·후보 풀·프롬프트 문구·`PROMPT_VERSION`·`DEFAULT_MAX_DELTAS` 변경 금지**(이번 라운드).
    ...
    → **2026-09-19 전량 철회(사용자 지시).** 최종 목표는 9월 20일이고, 26.19가 바꾸는 것은
    **데이터뿐**이지 이 결함들의 성질이 아니다 — "캐시가 원래 없는 시점에 묶는다"는 것은 비용을
    미루는 논거이지 품질 논거가 아니었다. 같은 라운드 안에서 후보 풀 정화 + `PROMPT_VERSION` v4
    (커밋 `bcd174c`)를, 이어서 프롬프트 v5 + 길이 재요청을 실행했다. 수용 기준과 롤백 조건은
    `ACCEPT-prose-v5-2026-09-19.md`에 **실행 전에** 고정했다. `DEFAULT_MAX_DELTAS`는 그대로 120 —
    상한을 올릴 근거가 이번 라운드에 없다(캐시 비용과 무관한 별개 판단).
  - **`--no-llm` 재매칭 금지.** 기존 LLM 원인 문장 233건이 소멸한다.
  - `MatchStatus` 열거·`EFFECT_SIZE_FLOORS`·`verdict.ts` 판정 규칙·`pubg-delta.ts`·`next.config.ts` 무수정.
  - ~~방법론의 `id="discord"` 섹션을 되살리지 않는다. 링크의 프래그먼트만 뗀다.~~
    → **2026-09-19 의도적 번복(독립 채점 K1-2).** 프래그먼트만 떼자 "방송 규칙 보기 →"가 방송
    이야기가 0건인 페이지에 착지했다. 2026-09-14에 제거된 것은 임베드 모양을 흉내 낸 **미리보기
    목업**(`DiscordEmbedPreview.tsx`·`discordPreview.ts`)이고, 이번에 넣은 것은 "무엇이 언제
    나가나"라는 **사실 서술**이다 — 제거 사유(가짜 미리보기)와 추가 사유(실제 동작 설명)가
    충돌하지 않는다. 목업 컴포넌트는 재도입하지 않는다.
    → **확인 대기 종결(2026-09-19).** 처음에는 "사용자 확인 필요"로 올렸으나, 사용자가 이 항목이
    무엇인지 되물었다 — 확인을 요청하는 것 자체가 비용이라는 뜻이다. 위 근거(제거 사유와 추가
    사유가 다르다)로 **결정으로 닫는다**. 되돌릴 일이 생기면 `git revert`로 한 커밋이다.
...
## 4-2. 항목 7(LLM 문장) 근본해결 실측 — 프롬프트 v4 + 후보 풀 정화

정확성 수정을 커밋해 안전지대를 만든 뒤 별도 단계로 실행했다. 후보 풀을 core 노트로 좁히고
(26.17: 215→42 · 26.18: 180→45 · 26.16: 278→63) 길이 상한을 어림수에서 숫자로 바꾼 뒤
`PROMPT_VERSION`을 v4로 올려 전량 재생성했다(캐시 무효는 의도된 대가).

| 지표 | 수정 전 | 모드 게이트만 | **v4 적용 후** |
|---|---|---|---|
| 26.17→26.18 검증 통과 요약 | 102건 | 102건 | **110건** |
| 그중 80자 초과 | 72건(71%) | 72건 | 36건(33%) — **100자 초과는 2건** |
| 최장 요약 | 124자 | 124자 | **107자** |
| 검증 통과 원인 | 206건 | 206건 | **221건** |
| 그중 80자 초과 | 38건 | 38건 | **0건**(최장 72자) |
| 원인 완곡 종결 | 169건(82%) | 169건 | 92건(42%) |
| 26.16→26.17 검증 통과 원인 | 460건(282건이 모드 인용) | 131건 | **245건**(모드 인용 0) |

두 가지가 동시에 확인됐다. ① 길이 상한은 **숫자로 못박으니 지켜졌다**(원인 0건 초과). 어림수
"80자 안팎"으로는 71%가 초과했다. ② 후보 풀을 정화하자 **검증 통과 원인이 오히려 늘었다**
(26.16→26.17 131→245건) — 모델이 인용할 수 없는 후보를 보지 않게 되자 실제 SR 조항을 더 찾았다.
사전에 우려한 "커버리지 하락"은 반대로 나타났다.

실지출: 두 쌍 합계 230콜 · 캐시 읽기 1.37M · 출력 120k 토큰.
