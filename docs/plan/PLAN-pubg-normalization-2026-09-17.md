...
- [ ] 7,217건 전량의 모든 무기 키가 분류되고, 미분류는 throw로 CI에서 잡힌다
- [ ] 스킨 변종 3종이 베이스에 합산되어 별도 행으로 노출되지 않는다
- [ ] 판정 목록에 `firearm`이 아닌 항목이 0건 (`Mortar` 포함)
- [ ] §8 반증표가 커밋 코드로 재현되고, **출하 축이 아님이 코드·화면·문서 3곳에 명시**
- [ ] 변동된 판정 숫자가 화면 문구·방법론에 동반 반영
- [ ] `verify.sh --full` PASS · 배포본 내용 재검증 PASS
