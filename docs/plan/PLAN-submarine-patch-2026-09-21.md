...
| TFT | 18.1 → 18.2 | 유닛·아이템 수치 변경 44건 | **37건**(유닛 19 · 아이템 18) |
| PUBG | 42.3 → 43.1 | 무기 59종 · 부위 셀 191개 피해 격자(전량 5,079매치) | **0건** — 분포 이동 검정으로 확정(§2-1 PUBG 절) |
...
**TFT** — 장로 드래곤 `damage 110→125`(+13.6%)·`armor 70→75`·`magicResist 70→75`, 조약돌
`damage 30→35`, 아무무 `mana 140→125`. TFT 노트가 「소규모 변경 사항」에 전부 적지 않는다는 실측이다.

⚠️ **숫자 정정(2026-09-21, 구현 후)**: 착수 전 분석은 32건(유닛 5·아이템 27)이었으나 구현 결과는
**37건(유닛 19·아이템 18)**이다. 방법이 달라서다 — 사전 분석은 *엔티티가 노트에 언급됐는가*로
보는 조잡한 방식이었고, 구현은 §3이 정한 **필드 단위** 대조다. 그래서 더 찾는다:
- 카직스 — 노트는 `기본 공격력 30→40`만 말하는데 **체력 850→950**이 바뀌었다.
- 레오나 — 노트는 마나·스킬 방어력을 말하는데 **체력 700→750**이 바뀌었다.
- 마스터 이 — 노트는 `기본 공격력 65→60`을 말하는데 **방어력 60→55**가 바뀌었다.
폭풍갈퀴와 정확히 같은 패턴이다(엔티티는 노트에 있지만 그 줄이 다른 수치를 말한다). 사전 분석
숫자가 낡은 것이지 구현이 틀린 게 아니므로 **기준선을 갱신한다**.
...
- TFT 18.1→18.2: 잠수함 **37건**(유닛 19 · 아이템 18)이고 장로 드래곤·조약돌·아무무가 그 안에 있다.
- PUBG 42.3→43.1: 전량 5,079매치, 무기 59종·부위 셀 191개에서 **격자 이동 0건**, 잠수함 **0건**.
  판별자는 분포 이동 검정(로그 구간 0.4% 히스토그램 교집합의 이동 이득 D ≥ 0.10, 이동 ≥ 1.2%)이고
  다음 세 대조로 보정됐다 — 무변화 대조군(같은 패치 무작위 반반, 381셀) D 최대 0.021 · 실제 쌍 D 최대
  0.027 · 합성 이동(×1.03·×0.97·×1.10) D 중앙값 0.42. 실측 픽스처(AK47 몸통·UMP 몸통·DP12 다리)로
  회귀 고정(`__tests__/fixtures/pubg-grid-42.3-43.1.json`).
- PUBG 노트는 `entity`가 없고 `weaponKeys`를 갖는다 — `expandPubgNotes`로 펴서 대조한다(안 펴면 전부
  잠수함이 되는 반대 방향 오탐). 잠수함 `entityKey`는 판정표와 같은 정준키(`Item_Weapon_*_C`)·표기명(SLR 등).
