...
### 마지막 구멍 — R10: 아무 워크플로도 F9를 부르지 않았다

R9로 스냅숏은 자동이 됐는데, 그 스냅숏을 **대조하는** `run-gamedata-diff`는 `collect.yml`·
`collect-tft.yml`·`collect-pubg.yml` 어디에서도 호출되지 않았다(실측: `grep -rn "gamedata-diff"
.github/` 0건). 다음 패치가 오면 `loadGameDataDiff(game, from, to)`가 새 쌍에 `null`을 돌려주고
`{submarine ? <SubmarineSection/> : null}`이 **세 게임 모두에서** 섹션을 통째로 지운다. 즉 이 기능은
사람이 매 패치 손으로 돌려야만 살아 있는 상태였다.

**막고 있던 것은 버전 짝이었다.** cron은 "지금 라이브인 패치"(26.19·18.3)만 알고, 수치 스냅숏은
게임 버전(16.19.1·16.19)으로 이름이 붙는다. 그 대응은 규칙이 아니라 사실이라 계산할 수 없다 —
LoL 26.18 ↔ 16.18.1, TFT 18.2 ↔ 16.18은 서로 다른 축이다.

- **R10. 버전 짝을 저장소에서 찾는다.** 새 매핑 파일을 만들지 않는다 — **그 사실은 이미 커밋돼
  있다**. `versionFrom`은 직전 쌍 산출물의 `meta.source.to`(지난 실행이 적어 두고 갔다),
  `versionTo`는 같은 잡의 앞 스텝이 방금 받아 둔 가장 새 스냅숏이다. 규칙은 순수 모듈
  `src/pipeline/gamedata/snapshot-version.ts`, 디스크는 `scripts/shared/snapshot-version.ts`.
  두 버전이 **같으면 던진다** — 조용한 0건은 화면에서 "잠수함 없음"과 구분되지 않는다.

**세 워크플로 전부에 붙였다(LoL 포함).** 자리는 판정·브리핑 **뒤**, 커밋 **앞**이고
`continue-on-error: true`다.
- 뒤여야 하는 이유는 입력이다 — LoL은 `run-match.ts`가 `notes/{to}.json`을 만든다.
- `continue-on-error`인 이유는 폭발 반경이다. 9/24 무인 실행이 예선 심사 기간(9/21~10/5) 안에
  있고 사이트 다운은 자격요건 위반이다. 잠수함은 **있으면 더 보여주는** 축이라 수집·판정·커밋을
  막을 자격이 없다. 깨지면 로그에 남고 그 패치의 그 섹션만 빈다 — 지금과 같은 상태다.
- PUBG 게이트에는 `notes_ready`도 넣는다. 노트가 수기 입력이라 며칠 늦게 들어오고, 그 사이
  `runPubg`가 `notes-{to}.json`을 못 찾아 매일 던진다.

`collect.yml`은 **기존 스텝을 한 줄도 고치지 않았다**(git add가 이미 `data/aggregated` 전체라
경로 추가도 불필요). 제약 원문이 「`collect.yml`의 LoL **동작 회귀** 금지」이므로 새 스텝 추가는
그 안에 있고, 그럼에도 회귀 표면을 0으로 두는 쪽을 골랐다.

**검증은 재현으로 했다** — 세 게임 산출물을 버전 인자 **없이** 재생성해 커밋본과 대조했고
`generatedAt` 외 전부 동일했다(lol 26.17_26.18 → 16.17.1→16.18.1 · tft 18.1_18.2 → 16.17→16.18 ·
pubg 42.3_43.1). 다음 패치 해소도 실측으로 확인했다: `versionFrom(lol, 26.18) = 16.18.1` ·
`versionFrom(tft, 18.2) = 16.18`.
