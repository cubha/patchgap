...
## 8. 요구사항 갱신 (2026-09-20, 사용자 추가 지시)

이 계획은 "TFT·PUBG 정기 수집을 CI에 배선한다"로 시작했다. 진행 중 사용자가 두 가지를 추가했고,
그 둘이 이번 라운드의 **더 큰 몫**이 됐다. 기준선을 갱신해 둔다(갱신하지 않으면 다음 인수검증에서
허위 MISSING이 난다 — sh-dev-loop 1-4-2).

### 8-1. TFT LLM 원인 분석 (신규 · 최우선)

> "지금 TFT내용이 대부분 이런식으로 정보만 표기되고 LLM분석을 통한 원인 유추는 안 하는거같음.
> 이 시스템 목적이 LLM분석을 통한 원인분석 → 사용자에게 자연어로 예상원인 등 전달하는건데
> 이런식으로 표기되는건 아닌거같음"

실태: TFT 델타 556행의 `causes`가 **0건**, `meta.llm`이 `null`. `run-tft-match.ts`에 LLM 참조가
한 줄도 없었다. 화면은 "무근거는 회색" 규칙대로 원인 줄을 조용히 생략하고 있었다.

수용 기준:
- TFT 파이프라인이 LoL과 **같은 엔진**으로 2단·3단을 돈다(새 판정 술어를 만들지 않는다).
- **LoL 프롬프트·캐시가 바이트 단위로 보존된다.** 캐시 키에 프롬프트 본문이 없으므로
  히트율이 아니라 문자열 golden으로 증명한다.
- 원인이 **화면에 보인다** — 상세·홈·대조표 세 표면에 각각 배선한다(생성만 하면 화면은 그대로다).
- 평균 등수의 개선 방향이 프롬프트·화면·짝짓기 **세 곳에서 같은 말**을 한다.

### 8-2. 랜딩 카드 그리드 (§7의 제외 합의를 철회)

> "랜딩 카드 그리드도 카드 추가되는걸 생각해서 미리 작업진행해"

§7은 "게임 4개째를 붙일 때 같이"로 미뤄 두었는데, 사용자가 **지금 하라고** 지시했다. 근거도
바뀌었다: 미룬 이유가 "심사 기간 중 무이득 변경 회피"였지만, `auto-fit` 그리드는 카드가 5장이
되는 순간 줄이 늘어나므로 **다음 게임을 붙이는 작업과 묶이면 그때 또 밀린다**.

수용 기준: 카드 수가 늘어도 이 영역의 크기가 변하지 않고 내부에서 가로 스크롤한다.
검증은 **부풀려서** 한다(카드 4장으로 찍은 화면은 아무것도 증명하지 않는다).

### 8-3. §6 검증 계획의 부분 미이행 — 정직 고지

§6은 "신규 워크플로 **둘 다** 실제 dispatch 실행을 VERIFY에 포함한다"고 적었다. TFT는 이행했다
(run 35515025960 성공). **PUBG는 이행하지 못했다** — `determine`이 오늘 날짜에서
"보존창을 넘겼다"로 정확히 멈추고, 다음 PUBG 패치가 ≈10/8로 예선 기간(~10/5) 밖이다.
대신 각 구성요소를 로컬 실호출로 검증했다(라이브 수집 3건 · 표본 가드 bash 실데이터 재현 ·
집계 무회귀 바이트 비교 · Python 산출과 19개 필드 일치). 워크플로 **전체**의 실행 증명은 없다.

### 8-4. 구현 파일 배치가 §5 SubTask 표와 다르다 (기준선 정정)

§5는 PUBG 이식을 파일 3개로 나눠 적었다. 실제 구현은 2개로 합쳤다 — **기준선 쪽을 정정한다**
(코드를 계획에 맞추려고 파일을 쪼개는 것은 목적을 잃은 준수다).

| §5 계획 | 실제 | 합친 근거 |
|---|---|---|
| `collect/pubg/reduce.ts` | `collect/pubg/telemetry-reduce.ts` | 이름만 다르다(축약 대상을 이름에 남겼다) |
| `collect/pubg/match-id.ts` | 같은 파일의 `parseMatchDefinition` | 패치 라벨은 `LogMatchDefinition` **이벤트**에서 나온다. 축약기가 이미 그 이벤트를 훑고 있어, 따로 두면 축약기만 import하는 10줄 모듈이 생긴다 |
| `collect/pubg/harvest.ts` | `scripts/run-pubg-collect.ts` | harvest I/O 계층의 내용이 "표본 id 수집 → 매치 → 텔레메트리 → 축약" 루프와 워커 풀뿐이라, 진입점과 분리하면 한 쪽이 다른 쪽만 부르는 껍데기가 된다 |

**README 함정 #1(표본 절반이 비경쟁 매치)은 ST6의 실질이고, 그것은 이행됐다** —
`parseMatchDefinition`이 `matchType`을 뽑아 축약본에 남기고 `selectMatches`가
`matchType === "official"`로 거른다. 축약 단계에서 버리지 않는 이유는 그 비율 자체가 표본
정의의 일부이기 때문이다(`telemetry-reduce.test.ts`가 `airoyale` 라벨을 읽는지 고정한다).

### 8-5. PUBG에 프리플라이트를 만들지 않는다 (§3-⑤의 범위 확정)

§3-⑤("키 미승인 상태에서 cron이 발화하면 어떻게 실패하나")는 **Riot 키 문제**였다 — TFT 개발
키가 24시간마다 만료되고 제품 승인이 걸려 있어서다. PUBG 키에는 만료도 제품 승인도 없다.

대신 **336시간 보존창이 그 자리를 대신한다**: 창을 넘긴 날짜는 텔레메트리가 404라 돌아도
빈손이므로, `determinePubgRun`이 들어가기 전에 막고 그 사유를 말한다. 즉 "싼 호출 하나로
오늘 돌 수 있는지 먼저 판정한다"는 §3-⑤의 **목적**은 충족하되 수단이 다르다.
계획이 이 면제를 명시한 적이 없으므로 여기 적어 둔다.
