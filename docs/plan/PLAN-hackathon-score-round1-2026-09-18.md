...
| ST-3 | — | llm-match: `LLM_MODEL="claude-opus-5"`, `PROMPT_VERSION="v3"`, 규칙 추가(사용자용 문장 — "제공된/후보 목록" 언급 금지, 비율은 %·%p, 영문 키 병기 금지, 한 문장 60자 내외). `run-match` `--llm-max` 기본 120. 재실행 26.17→26.18 · 26.16→26.17 → deltas·캐시 커밋 | src/pipeline/match/llm-match.ts · scripts/run-match.ts · data/aggregated/deltas/*.json · data/cache/llm/* | D3 D4 |
